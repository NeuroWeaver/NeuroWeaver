...
class ActorCriticPolicy(BasePolicy):
    ...
    def forward(self, obs: th.Tensor, deterministic: bool = False) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        """
        #print("forward using our own ActorCriticPolicy policy, do whaever we want here!")
        # Preprocess the observation if needed
        features = self.extract_features(obs)
        latent_pi, latent_vf = self.mlp_extractor(features)
        # Evaluate the values for the given observations
        # emascare: value_net is the torch Linear model.
        # replace this computation step with genesys computation
    ...
    def forward_fpga(self, obs: th.Tensor, driver: GenesysDriver, deterministic: bool = False ) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        """
                Forward pass in all the networks (actor and critic)
                ...
                :return: action, value and log probability of the action
                """
        print("forward using our own ActorCriticPolicy policy forward for fpga!")
        # Preprocess the observation if needed
        features = self.extract_features(obs)
        latent_pi, latent_vf = self.mlp_extractor(features)
        # value_net is the torch Linear model.
        # replace this computation step with genesys computation
        values = self.value_net(latent_vf)
        # TODO: Quantize inputs and then update it
        #driver.update_inputs(latent_vf.numpy())
        driver.init_buffers()
        driver.program_registers()
        driver.run()
        driver.check_output()
        print("FPGA Commute Complete")
        # TODO: Dequntize output if required, split host_output_buffer 32-bit into 4 8-bit values, convert it to Pytorch tensor
        #values = th.Tensor(driver.host_output_buffer)
        distribution = self._get_action_dist_from_latent(latent_pi)
        actions = distribution.get_actions(deterministic=deterministic)
        log_prob = distribution.log_prob(actions)
        return actions, values, log_prob
