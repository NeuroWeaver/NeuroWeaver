...
// ------------------------------------------------------------------------------------
    ...
    */
    ...
    cl::Buffer base_address(context, CL_MEM_ALLOC_HOST_PTR | CL_MEM_READ_ONLY, TOTAL_DATA_SIZE, NULL, &err);
    cl::Buffer outputs(context, CL_MEM_ALLOC_HOST_PTR | CL_MEM_READ_WRITE, ADDR_OFFSET_OUTPUT + sizeof(int) * NUM_OUTPUT, NULL, &err);
    cl::Buffer simd_outputs(context, CL_MEM_ALLOC_HOST_PTR | CL_MEM_READ_WRITE, MAX_OFFSET_SIMD + sizeof(int) * NUM_OUTPUT, NULL, &err);
    
    // Map host-side buffer memory to user-space pointers (kernel paramters?)
    int *base_ptr = (int *)q.enqueueMapBuffer(base_address, CL_TRUE, CL_MAP_WRITE, 0, TOTAL_DATA_SIZE);
    int *output_ptr = (int *)q.enqueueMapBuffer(outputs, CL_TRUE, CL_MAP_WRITE | CL_MAP_READ, 0, ADDR_OFFSET_OUTPUT + sizeof(int) * NUM_OUTPUT);
    int *simd_output_ptr = (int *)q.enqueueMapBuffer(simd_outputs, CL_TRUE, CL_MAP_WRITE | CL_MAP_READ, 0, MAX_OFFSET_SIMD + sizeof(int) * NUM_OUTPUT);
    ...
    //initialize_array(simd_output_ptr, MAX_OFFSET_SIMD + sizeof(int) * NUM_OUTPUT, reg_initialize_val);
...
// ------------------------------------------------------------------------------------
    ...
    bool match2 = true;
    bool match0 = true;
    int output_buffer_i = 0; // since we have 8 bit numbers, this only go up every 4 num_output count
    ...
    std::cout << "------------vmem2---------\n";
    output_buffer_i = 0; // since we have 8 bit numbers, this only go up every 4 num_output count
    for (int i =  0  ; i < num_output ; i++){
        int expected = output_reference_arr[i];
        int output_32bit = simd_output_ptr[output_buffer_i+(simd_addr_vmem2_iterator)];
    ...
    for (int i =  0  ; i < num_output ; i++){
        int expected = output_reference_arr[i];
        int output_32bit = simd_output_ptr[output_buffer_i+(simd_addr_vmem1_iterator)];
        int eightbit_count = i % 4;
        uint8_t device_out = output_32bit >> (eightbit_count * 8);
        if (eightbit_count == 3) {
            output_buffer_i += 1;
        ...
        // std::cout << "i = " << i << " CPU result = " << expected << " Device result = " << (int) device_out << std::endl;
        // std::cout << " Device 32 bit result = " << output_32bit << std::endl;
    ...
    std::cout << "TEST " << ((match1) ? "PASSED" : "FAILED") << std::endl;
    

    q.enqueueMigrateMemObjects({base_address}, CL_MIGRATE_MEM_OBJECT_HOST,NULL,&timing_event);
    q.finish();

    //  output_addr_iterator =  base_ptr;
    int count = 0;
    for (int i =  0  ; i < 24 ; i+=4) { // 4 byte per stats, 6 stats
        int local_i = i ;
        int value = 0;
        if (count ==0){
