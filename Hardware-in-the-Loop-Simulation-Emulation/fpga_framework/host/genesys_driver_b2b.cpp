...
// ------------------------------------------------------------------------------------
    ...
    */
    ...
    // cl::Buffer outputs(context, CL_MEM_ALLOC_HOST_PTR | CL_MEM_READ_WRITE, 1024, NULL, &err); // output buffer is not used 
    // cl::Buffer simd_outputs(context, CL_MEM_ALLOC_HOST_PTR | CL_MEM_READ_WRITE, sizeof(int) * (MAX_OFFSET_SIMD + NUM_OUTPUT), NULL, &err);

    cl::Buffer base_address(context, CL_MEM_ALLOC_HOST_PTR | CL_MEM_READ_WRITE, sizeof(int) * TOTAL_DATA_NUM, NULL, &err);
    int *base_ptr = (int *)q.enqueueMapBuffer(base_address, CL_TRUE, CL_MAP_WRITE | CL_MAP_READ, 0, sizeof(int) * TOTAL_DATA_NUM);
    ...
    // int *output_ptr = (int *)q.enqueueMapBuffer(outputs, CL_TRUE, CL_MAP_WRITE | CL_MAP_READ, 0, 1024);
    // int *simd_output_ptr = (int *)q.enqueueMapBuffer(simd_outputs, CL_TRUE, CL_MAP_WRITE | CL_MAP_READ, 0, sizeof(int) * (MAX_OFFSET_SIMD + NUM_OUTPUT));
    
// ------------------------------------------------------------------------------------
// Step 3: Read instruction and input files to the respective buffers 
// ------------------------------------------------------------------------------------
    std::cout << "Reading Test Folder Hiearchy\n";
    ...
    for (layer_iter = data_info.begin(); layer_iter != data_info.end(); ++layer_iter) {
        ...
        for (input_iter = layer_input.begin(); input_iter != layer_input.end(); ++input_iter) {
            ...
            std::cout << "INFO: file offset: " << (*input_iter)["offset"].asInt() << "\n"; 
    ...
    std::string instruction_file_path = "";
    std::string instruction_file_name = "";
    std::string directory_name = "";
    std::string inst_paths[NUM_LAYER];

    // Fill Instruction
    std::cout << "Load Layer Instruction\n";
    while (entry != NULL) {
      if (entry->d_type == DT_DIR &&((std::string) entry->d_name) != "program" &&((std::string) entry->d_name) != ".." &&((std::string) entry->d_name) != "."){
        num_blocks++ ;
        directory_name = (std::string) entry->d_name ;
        instruction_file_path = instruction_file_path + test_path + directory_name + "/" ;
        int layer_num_pos = instruction_file_path.find("layer")+5;
        int layer_num = instruction_file_path[layer_num_pos] - '0';
    	instruction_file_name = directory_name.substr(directory_name.find('_')+1,-1) + "_decimal.txt" ;
        instruction_file_path = instruction_file_path + instruction_file_name ;
        inst_paths[layer_num] = instruction_file_path;
    ...
    // PPO insturction
    // int instr_offset = 0;
    // for (int i = 0 ; i<3 ;i++) { // order in reverse
    //     // std::cout << i << " " << instr_offset << " " << inst_paths[i] << "\n"; 
    //     read_instructions_file(inst_paths[i], instr_offset, base_ptr, 0);
    //     if (i == 0) {
    //         instr_offset += 53248/4; // for PPO
    //     } else {
    //         instr_offset += 4096/4;
    //     }
    // }
    ...
    // SAC
    int instr_offset = 0;
    for (int i = 0 ; i<NUM_LAYER ;i++) { // order in reverse
        std::cout << i << " " << instr_offset << " " << inst_paths[i] << "\n"; 
        read_instructions_file(inst_paths[i], instr_offset, base_ptr, 0);
        instr_offset += 4096/4;
    }
...
// // ------------------------------------------------------------------------------------
    ...
    for (layer_iter = data_info.begin(); layer_iter != data_info.end(); ++layer_iter) {
        ...
        for (output_iter = layer_output.begin(); output_iter != layer_output.end(); ++output_iter) {
            ...
            std::cout << "Output offset: " << (*output_iter)["offset"].asInt() << "\n";
            read_data_file(path, output_reference_arr, 0, 1);
            std::cout << "Num output " << num_output << "\n";
            //int output_offset = 0;
            ...
            for (int i =  0  ; i < num_output ; i++){
                int expected = output_reference_arr[i];
                // int output_32bit = simd_output_ptr[output_buffer_i+output_offset];
                int output_32bit = base_ptr[output_buffer_i+output_offset];
                int eightbit_count = i % 4;

                uint8_t device_out = output_32bit >> (eightbit_count * 8);
                if (eightbit_count == 3) {
                    output_buffer_i += 1;
                }

                // if (eightbit_count == 0) {
                //     std::cout << "32bit out " << output_32bit << std::endl;
                // }

                if (abs(((int) device_out) - expected) > 0) {
                    std::cout << "i = " << i << " CPU result = " << expected << " Device result = " << (int) device_out << std::endl;
                    match[layer_count] = 0;
                    //break;
                }
            }
            ...
            for (int i =  0+pc_offset  ; i < 24 + pc_offset ; i+=4) {
                int local_i = i ;
                int value = 0;
                if (count == 0) {
            ...
            pc_offset += 24;
            std::cout << "TEST " << ((match[layer_count] == 1) ? "PASSED" : "FAILED") << std::endl; 
        }
        layer_count+=1;
    }
    ...
    delete[] fileBuf;
    // return EXIT_FAILURE ;
    return (match[2] == 1 || match[3] == 1) ? EXIT_SUCCESS : EXIT_FAILURE;
}
