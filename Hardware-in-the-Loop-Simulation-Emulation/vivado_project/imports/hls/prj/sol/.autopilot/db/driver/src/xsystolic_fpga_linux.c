...
typedef struct {
    ...
    XSystolic_fpga_uio_map maps[ MAX_UIO_MAPS ];
} XSystolic_fpga_uio_info;

/***************** Variable Definitions **************************************/
static XSystolic_fpga_uio_info uio_info;
...
static int uio_info_read_name(XSystolic_fpga_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XSystolic_fpga_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XSystolic_fpga_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XSystolic_fpga_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XSystolic_fpga_Initialize(XSystolic_fpga *InstancePtr, const char* InstanceName) {
	XSystolic_fpga_uio_info *InfoPtr = &uio_info;
