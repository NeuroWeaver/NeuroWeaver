...
fh = logging.FileHandler('debug_buffers.log')
...
class ReplayBuffer(BaseBuffer):
    ...
    def add(
		...
		# in our osc_env, infos are empty, infos are default as None
        ...
        self.pos += 1
        logger.debug(f"self.pos {self.pos} == self.buffer_size {self.buffer_size}")
        if self.pos == self.buffer_size:            
            logger.debug("!!self.pos == self.buffer_size!!")
            self.full = True
            self.pos = 0
