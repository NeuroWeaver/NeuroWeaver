...
with Component(outputs=[tin, weights, input_im, input_jpg, tfnet]) as init:
...
with Component(inputs=[tfnet, box_input, input_im], outputs=[boxes_info]) as bbox_drawer:
    h, w, _ = input_im.shape
    boxes = tfnet.framework.findboxes(box_input[0])
...
with Component(inputs=[boxes_info, input_im, input_jpg]) as display:
...
tin, weights, input_im, input_jpg, tfnet = init()
tout = yolo_dnn(tin, weights)
boxes_info = bbox_drawer(tfnet, tout, input_im)
display(boxes_info, input_im, input_jpg)
