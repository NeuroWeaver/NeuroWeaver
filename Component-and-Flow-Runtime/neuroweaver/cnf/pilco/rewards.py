...
class ExponentialReward(Module):
    def __init__(self, all_states, state_dim, W=None, t=None):
        ...
        if t is not None:
            ...
            #Modified the reward
            self.t = Parameter(np.mean(all_states), trainable=False)
            #self.t = Parameter(np.mean(np.mean(all_states[:,:-2],axis=1)), trainable=False)
            #self.t = Parameter(np.zeros((1, state_dim)), trainable=False)

    def compute_reward(self, m, s, m_action):
        '''
        ...
        # r2 with action consideration:
#        r2 =  tf.exp(-((m-self.t) @ i2SpW @ tf.transpose(m-self.t))-tf.math.scalar_mul(2, (m_action @ tf.transpose(m_action)))) / \
#                tf.sqrt( tf.linalg.det(tf.eye(self.state_dim, dtype=float_type) + 2*SW) )
        r2 =  -((m-self.t) @ i2SpW @ tf.transpose(m-self.t))-tf.math.scalar_mul(2, (m_action @ tf.transpose(m_action)))
