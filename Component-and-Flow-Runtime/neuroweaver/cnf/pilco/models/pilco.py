...
class PILCO(gpflow.models.BayesianModel):
    def __init__(self, all_states, data, num_induced_points=None, horizon=30, controller=None,
        ...
        if reward is None:
            self.reward = rewards.ExponentialReward(all_states, self.state_dim)
        else:
            self.reward = reward
        ...
        if m_init is None or S_init is None:
            ...
            self.S_init = S_init
        self.optimizer = None
        self.u_init = tf.constant([[0]], float_type)
    def training_loss(self):
        # This is for tuning controller's parameters
        reward = self.predict(self.m_init, self.S_init, self.u_init, self.horizon)[2]
        return -reward
    ...
    def predict(self, m_x, s_x, m_u, n):
        loop_vars = [
        ...
        _, m_x, s_x, m_u, reward = tf.while_loop(
            # Termination condition
            lambda j, m_x, s_x, m_u, reward: j < n,
            # Body function
            lambda j, m_x, s_x, m_u, reward: (
                j + 1,
                *self.propagate(m_x, s_x),
                tf.add(reward, self.reward.compute_reward(m_x, s_x, m_u)[0]) # Parisa- added m_u : , self.controller.compute_action(m_x, s_x)[0] didn't work yet!
            ), loop_vars
        )
